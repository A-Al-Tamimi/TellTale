...
Three probability streams over the interview transcript, blended:
  ...
  J  Qwen3-14B (4-bit, zero-shot prompt via mlx-lm), no training
...
CHUNK_SEED = 0   # chunk-level pretraining uses one fixed seed per fold; the
                 # MIL continuation is what varies across the stream's seeds

STREAMS = {
    "a": {
        "model": "intfloat/multilingual-e5-large",
        "prefix": "query: ", "pool": "mean", "max_len": 256, "head": "mlp",
    ...
    "b": {
        ...
        "prefix": "", "pool": "cls", "max_len": 128, "head": "linear",
...
# TODO: CoT prompts with a few shots. 
JUDGE_SYSTEM = (
...
def video_split(df, seed):
    """Participant-grouped 90/10 split at the video level (used for MIL)."""
...
def chunk_split(df, seed):
    """Participant-grouped 90/10 split at the chunk level (used for chunk-level
    pretraining), matching the original stream-A training."""
    from sklearn.model_selection import GroupShuffleSplit
    gss = GroupShuffleSplit(n_splits=1, test_size=0.1, random_state=seed)
    itr, iva = next(gss.split(df, df["chunk_label"], df["pid"]))
    return df.iloc[itr], df.iloc[iva]


def predict_chunk_probs(model, tokenizer, spec, texts, device):
    import torch
    ...
    with torch.no_grad():
        for i in range(0, len(texts), 32):
            enc = encode(tokenizer, spec, texts[i:i + 32], dev)
            out.append(torch.sigmoid(model(enc)).cpu().numpy())
    return np.concatenate(out) if out else np.zeros(0, "float32")


def train_chunk_stage(model, tokenizer, spec, tr_df, va_df, device):
    """Chunk-level pretraining with chunk-level early stopping (stream A)."""
    import torch
    ...
    va_texts = va_df["text"].tolist()
    va_y = va_df["chunk_label"].values.astype(int)
    ...
    for ep in range(cfg["epochs"]):
        ...
        _, f1 = best_threshold(va_y, predict_chunk_probs(model, tokenizer, spec,
                                                         va_texts, device))
        print(f"  chunk stage ep{ep + 1}: val chunk Macro-F1={f1:.4f}", flush=True)
        if f1 > best["f1"]:
            best = {"f1": f1, "ep": ep,
...
def train_mil_stage(model, tokenizer, spec, tr_df, va_df, vid2label, device):
    ...
    for ep in range(cfg["epochs"]):
        ...
        for i in tqdm(range(0, len(tr_vids), 4),
                      desc=f"mil ep{ep + 1}/{cfg['epochs']}"):
...
def chunk_base(stream, spec, tokenizer, chunks, fold, device):
    """Train (or load) the per-fold chunk-level pretrained base for a stream
    that uses chunk pretraining. Trained once per fold with CHUNK_SEED and
    reused by every MIL seed, matching the original stream-A pipeline."""
    base_dir = OUT_DIR / f"stream_{stream}" / f"fold{fold}_base"
    if (base_dir / "trainable.pt").exists():
        return load_trainable(spec, base_dir)
    set_seed(CHUNK_SEED)
    tr_all = chunks[chunks["fold"] != fold]
    tr, va = chunk_split(tr_all, CHUNK_SEED)
    model = build_model(spec)
    model = train_chunk_stage(model, tokenizer, spec, tr, va, device)
    save_trainable(model, base_dir, {"stream": stream, "fold": fold,
                                     "stage": "chunk_base"})
    return model
...
def cmd_train(args):
    ...
    for fold in folds:
        ...
        if spec["chunk_pretrain"]:
            base = chunk_base(args.stream, spec, tokenizer, chunks, fold, args.device)
        ...
        for seed in seeds:
            ...
            if probs_path.exists():
                ...
                continue
            set_seed(seed)
            tr_vids, va_vids = video_split(tr_all, seed)
...
# ---------------------------------------------------------------------------
# LLM judge; consider other judges in the future
# ---------------------------------------------------------------------------
